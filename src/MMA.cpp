...
PetscErrorCode MMA::KKTresidual(Vec x, Vec dfdx, PetscScalar* fx, Vec* dgdx,
  ...
  if (!NonLinConstraints) {
    PetscErrorPrintf(
        "MMA->KKTresidual called WITH constraints but object was "
        "allocated WITHOUT !\n");
    return -1;
  }
...
PetscErrorCode MMA::Update(Vec xval, Vec dfdx, PetscScalar* gx, Vec* dgdx,
  ...
  if (!NonLinConstraints) {
    PetscErrorPrintf(
        "MMA->Update called WITH constraints but object was "
        "allocated WITHOUT !\n");
    return -1;
  }
...
PetscErrorCode MMA::GenSub(Vec xval, Vec dfdx, PetscScalar* gx, Vec* dgdx,
  ...
  for (PetscInt i = 0; i < nloc; i++) {
    ...
    p0v[i] = pow(Uv[i] - xv[i], 2.0) *
             (dfdxp + 0.001 * Abs(dfdxv[i]) + 0.5 * feps / (Uv[i] - Lv[i]));
    q0v[i] = pow(xv[i] - Lv[i], 2.0) *
             (dfdxm + 0.001 * Abs(dfdxv[i]) + 0.5 * feps / (Uv[i] - Lv[i]));
    for (PetscInt j = 0; j < m; j++) {
      ...
      if (constraintModification) {
        pijv[j][i] =
            pow(Uv[i] - xv[i], 2.0) *
            (dfdxp + 0.001 * Abs(dgdxv[j][i]) + 0.5 * feps / (Uv[i] - Lv[i]));
        qijv[j][i] =
            pow(xv[i] - Lv[i], 2.0) *
            (dfdxm + 0.001 * Abs(dgdxv[j][i]) + 0.5 * feps / (Uv[i] - Lv[i]));
      } else {
...
PetscErrorCode MMA::XYZofLAMBDA(Vec x) {
  ...
  for (PetscInt i = 0; i < nloc; i++) {
    ...
    xv[i] = (sqrt(pjlam) * Lv[i] + sqrt(qjlam) * Uv[i]) /
...
PetscErrorCode MMA::DualHess(Vec x) {
  ...
  for (PetscInt i = 0; i < nloc; i++) {
    ...
    for (PetscInt j = 0; j < m; j++) {
      pjlam += pijv[j][i] * lam[j];
      qjlam += qijv[j][i] * lam[j];
      PQ[i * m + j] = pijv[j][i] / pow(Uv[i] - xv[i], 2.0) -
                      qijv[j][i] / pow(xv[i] - Lv[i], 2.0);
    }
    df2[i] = -1.0 / (2.0 * pjlam / pow(Uv[i] - xv[i], 3.0) +
                     2.0 * qjlam / pow(xv[i] - Lv[i], 3.0));
    PetscScalar xp = (sqrt(pjlam) * Lv[i] + sqrt(qjlam) * Uv[i]) /
                     (sqrt(pjlam) + sqrt(qjlam));
    if (xp < alf[i]) {
      df2[i] = 0.0;
    }
